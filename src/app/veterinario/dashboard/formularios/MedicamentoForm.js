...
export default function MedicamentoForm({ mascota, propietario, data = {}, editar = false, onSave, onComplete, onClose }) {
  const [formData, setFormData] = useState({
    ...
    precio: data.precio || 20, // Precio por defecto para medicamentos
  ...
  const [medicamentoGuardado, setMedicamentoGuardado] = useState(false);
  ...
  const handleChange = (e) => {
    ...
    } else if (["fecha", "diagnostico"].includes(name)) {
      delete newErrors[name];
    }
  ...
  const validarCita = () => {
    ...
    const servicioMedicamento = servicios.find((s) => s.nombre.trim().toLowerCase().includes("medicamento"));
    if (!servicioMedicamento) {
      return { isValid: false, error: "❌ No se encontró el servicio de Medicamento." };
    }

    return { isValid: true, fechaHora, servicioMedicamento };
  };
  ...
  const agendarCita = async () => {
    ...
    const { fechaHora, servicioMedicamento } = validacion;
    ...
    const cita = {
      ...
      servicio_id: servicioMedicamento.id,
  ...
  const marcarComoPagado = async () => {
    if (!data._id) {
      setErrors({ general: "ID del medicamento no definido." });
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await fetch(`https://mascota-service.onrender.com/api/mascotas/medicamentos/${data._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      setErrors({ success: "✅ Medicamento marcado como pagado con éxito." });
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("handleSubmit ejecutado - Inicio guardado de medicamento");
    setIsSubmitting(true);
    setErrors({});
    ...
    try {
      if (editar) {
        if (!data._id) {
          throw new Error("ID del medicamento no definido.");
        }
        const res = await fetch(`https://mascota-service.onrender.com/api/mascotas/medicamentos/${data._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        setErrors({ success: "✅ Medicamento actualizado con éxito." });
        setMedicamentoGuardado(true);
        ...
        setErrors({ success: "✅ Medicamento creado con éxito." });
        setMedicamentoGuardado(true);
        if (formData.proximo_control_fecha && formData.proximo_control_hora) {
      ...
      setErrors({ general: `❌ Error al guardar el medicamento: ${err.message}` });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Row>
          <Col md={6}>
            ...
            <Form.Group className="mb-3">
              <Form.Label><i className="bi bi-file-medical me-2"></i>Diagnóstico</Form.Label>
              <Form.Control
                as="textarea"
                list="diagnostico-suggestions"
                name="diagnostico"
              ...
              />
              <datalist id="diagnostico-suggestions">
                <option value="Infección bacteriana" />
                <option value="Dermatitis" />
                <option value="Gastroenteritis" />
                <option value="Insuficiencia renal" />
                <option value="Parásitos intestinales" />
              </datalist>
            </Form.Group>
            <Form.Group className="mb-3">
        ...
        <div className="d-flex justify-content-end mt-4">
          ...
          <Button
            ...
            {...buttonHover}
          >
            <i className="bi bi-save-fill me-2"></i>{editar ? "Guardar Cambios" : "Guardar Medicamento"}
          </Button>
        </div>
      </Form>
      ...
      <Modal
        ...
        onHide={() => {
          ...
          setMostrarModalConfirmacion(false);
          setIntentoCerrar(false);
          if (medicamentoGuardado || intentoCerrar) {
            if (editar) {
              onClose();
            } else {
              onComplete();
      ...
      >
        ...
        <Modal.Footer className="border-0">
          <Button
            ...
            {...buttonHover}
          >
            <i className="bi bi-x-circle-fill me-2"></i>No, solo guardar medicamento
          </Button>
          <Button
            variant="success"
            onClick={() => {
