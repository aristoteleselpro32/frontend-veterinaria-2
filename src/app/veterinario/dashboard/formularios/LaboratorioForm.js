...
export default function LaboratorioForm({ mascota, propietario, data = {}, editar = false, onSave, onComplete, onClose }) {
  ...
  const [laboratorioGuardado, setLaboratorioGuardado] = useState(false);
  ...
  const validarCita = () => {
    ...
    const servicioLaboratorio = servicios.find((s) => s.nombre.trim().toLowerCase().includes("laboratorio"));
    if (!servicioLaboratorio) {
      return { isValid: false, error: "❌ No se encontró el servicio de Laboratorio." };
    }

    return { isValid: true, fechaHora, servicioLaboratorio };
  };
  ...
  const agendarCita = async () => {
    ...
    const { fechaHora, servicioLaboratorio } = validacion;
    ...
    const cita = {
      ...
      servicio_id: servicioLaboratorio.id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editar) {
        if (!data._id) {
          throw new Error("ID del laboratorio no definido.");
        }
        ...
        if (!res.ok) throw new Error("Error al actualizar el análisis de laboratorio");
        setErrors({ success: "✅ Análisis de laboratorio actualizado con éxito." });
        setLaboratorioGuardado(true);
        ...
        setErrors({ success: "✅ Análisis de laboratorio creado con éxito." });
        setLaboratorioGuardado(true);
        if (formData.proximo_control_fecha && formData.proximo_control_hora) {
      ...
      setErrors({ general: `❌ Error al guardar el análisis de laboratorio: ${err.message}` });
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <div className="d-flex justify-content-end mt-3">
          ...
          <Button
            ...
            {...buttonHover}
          >
            <i className="bi bi-save-fill me-2"></i>{editar ? "Guardar Cambios" : "Guardar Laboratorio"}
          </Button>
        </div>
      </Form>
      ...
      >
        ...
        <Modal.Footer className="bg-dark border-secondary">
          <Button
            ...
            {...buttonHover}
          >
            <i className="bi bi-x-circle-fill me-2"></i>No, solo guardar laboratorio
          </Button>
          <Button
            variant="success"
            onClick={() => {
